...
export function useCustoEngine() {
  ...
  const construirContexto = useCallback(async (animalIds: string[]): Promise<ContextoCusto> => {
    if (!user || animalIds.length === 0) {
      return { animaisPorId: {}, pesagensPorAnimal: {}, periodosPorAnimal: {}, ciclos: [], dietas: {}, custosOperacionaisPorLote: {}, qtdAtivaPorLote: {} }
    }
    ...
    if (loteIdsEnvolvidos.size > 0) {
      ...
      const [{ data: ciclosData }, { data: custosOpData }, { data: ativosData }] = await Promise.all([
        supabase.from('ciclos_lote').select('lote_id, numero, dieta_id, gmd_esperado, data_inicio, data_fim').in('lote_id', loteIdsArr),
      ...
      for (const c of (custosOpData ?? []) as Array<{ lote_id: string; valor: number; data_lancamento: string }>) {
        (custosOperacionaisPorLote[c.lote_id] ??= []).push({ valor: c.valor, data_lancamento: c.data_lancamento })
      }
  ...
  const calcularEmLote = useCallback(async (
    ...
    for (const animalId of animalIds) {
      ...
      resultado[animalId] = calcularAnimalNaData(
        ...
        ctx.custosOperacionaisPorLote, ctx.qtdAtivaPorLote,
      )
    }
    return resultado
  }, [construirContexto])
