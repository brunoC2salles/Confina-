...
function calcularPonto(dia: number, peso_medio: number, gmd: number, input: InputProjecao): PontoProjecao {
  ...
  const peso_carcaca_total = peso * rendimento * input.qtd_animais
  ...
  const custo_total = input.custo_compra_total + input.custo_alimentacao_acumulado + (input.custo_alimentacao_dia * dia)
  ...
  return {
    dia, data: data.toISOString().split('T')[0],
