...
import type { CompraOutros } from '@/types'
import { buscarTudoPaginado, chunkArray } from './useLotes'

// ─── Compras "Outros" rateadas por cabeça ───────────────────────────────────
//
// Mesmo modelo de useComprasMedicamento.ts: uma compra que não é ração nem
// medicamento (ex.: sal mineral, frete, mão de obra pontual etc.), rateada em
// partes iguais entre os animais ATIVOS que estejam no ciclo alvo na data de
// aplicação — cruzando QUALQUER lote (todo o rebanho). O produtor descreve o
// que é a compra em `descricao`. Rateio fixo, calculado uma única vez no
// lançamento e gravado por animal em custos_variaveis_animal (mesma tabela
// que já alimenta o custo do animal em Ranking, Comparativo, Lotes e no
// cálculo de venda) — sem recálculo automático se a lista de animais do
// ciclo mudar depois.

const TAMANHO_CHUNK_INSERT = 500

export function useComprasOutros() {
  const { user } = useAuth()
  const [compras, setCompras] = useState<CompraOutros[]>([])
  ...
  const fetch = useCallback(async () => {
    ...
    const [{ data: comprasData }, { data: parceirosData }] = await Promise.all([
      supabase.from('compras_outros').select('*').eq('user_id', user.id).order('data_aplicacao', { ascending: false }),
      supabase.from('parceiros').select('id, nome').eq('user_id', user.id).eq('tipo', 'fornecedor').order('nome'),
    ])
    setCompras((comprasData ?? []) as CompraOutros[])
  ...
  const registrarCompra = async (input: {
    ...
    const { data: compra, error: eCompra } = await supabase.from('compras_outros').insert({
    ...
    const linhasCusto = animalIds.map(animal_id => ({
      ...
      data_lancamento: input.data_aplicacao, compra_outros_id: compra.id, user_id: user.id,
    }))
    for (const chunk of chunkArray(linhasCusto, TAMANHO_CHUNK_INSERT)) {
      ...
      if (eInsert) {
        ...
        // gravados. O CASCADE em compra_outros_id remove o que já tiver sido
        // inserido nos lotes anteriores.
        await supabase.from('compras_outros').delete().eq('id', compra.id)
  ...
  const excluirCompra = async (id: string) => {
    // ON DELETE CASCADE em custos_variaveis_animal.compra_outros_id remove
    // junto todos os lançamentos por animal gerados por essa compra.
    const { error } = await supabase.from('compras_outros').delete().eq('id', id)
  ...
  const buscarDistribuicaoPorLote = async (compraId: string): Promise<Array<{ lote_id: string; nome_lote: string; codigo_lote: string; quantidade: number }>> => {
    const linhas = await buscarTudoPaginado<{ animal_id: string; animais: { lote_atual_id: string | null; lotes: { nome_lote: string; codigo_lote: string } | null } | null }>((from, to) =>
      supabase.from('custos_variaveis_animal')
        ...
        .eq('compra_outros_id', compraId).range(from, to) as any)
