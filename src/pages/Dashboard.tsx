    ...
    const load = async () => {
      ...
      try {
        const [{ data: animais, error: e1 }, { data: saidas, error: e2 }, { count: qtdVendidos, error: e3 }] = await Promise.all([
          ...
          supabase.from('saidas_grupo').select('receita_liquida, custo_compra_total, custo_alimentacao_total, custos_variaveis_total, custos_fixos_rateados, lucro_total').eq('user_id', user.id),
        ...
        setMetricas({ totalAnimais, pesoMedioGeral, receitaTotal, custoTotal, lucroTotal, qtdVendidos: qtdVendidos ?? 0, valorCompraTotal })
      } catch {
