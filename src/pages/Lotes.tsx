...
import type { Lote, Animal, SaidaTipo, SaidaModo, RendimentoFaixa, BonusFaixa } from '@/types'
...
function FormLoteBase({
  ...
  // Campos de origem/preço só são pedidos quando NÃO é bifurcação — nesse caso herdam do lote de origem
  ...
  const validarStep = (): string | null => {
    if (step === 1) {
      ...
      if (!loteOrigem && (!precoKgCompra || Number(precoKgCompra) <= 0)) return 'Informe o preço por kg de compra'
    }
    if (step === 2) {
  ...
  const confirmar = async () => {
    ...
    const res = await onConfirmar({
      ...
      preco_kg_compra: loteOrigem ? (loteOrigem.preco_kg_compra ?? 0) : Number(precoKgCompra),
      origem_fazenda: loteOrigem ? (loteOrigem.origem_fazenda ?? undefined) : (origemFazenda || undefined),
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {loteOrigem && (
            <div style={{ padding: '10px 14px', background: 'var(--gray-50)', borderRadius: 8, fontSize: 12, color: 'var(--gray-500)' }}>
              Fazenda, município, estado e preço de compra são herdados do lote {loteOrigem.nome_lote}.
            </div>
          )}
...
function ModalAdicionarAnimais({
  ...
  lote: Lote
  onClose: () => void
  onConfirmar: (input: { data_entrada: string; data_pesagem2?: string; linhas: LinhaAnimalInput[]; origem?: string; raca?: string }) => Promise<{ error: string | null }>
}) {
...
function GraficoCustoReceita({ curva, diaIdeal }: { curva: PontoProjecao[]; diaIdeal: PontoProjecao | null }) {
  const dados = curva.map(p => ({ dia: p.dia, custo: p.custo_total, receita: p.receita_bruta }))
  return (
    <div style={{ width: '100%', height: 240 }}>
      <ResponsiveContainer>
        <LineChart data={dados} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
          <XAxis dataKey="dia" tick={{ fontSize: 11 }} label={{ value: 'Dias a partir de hoje', position: 'insideBottom', offset: -2, fontSize: 11 }} />
          ...
          {diaIdeal && (
            <ReferenceLine x={diaIdeal.dia} stroke="#2e7d32" strokeDasharray="4 4"
          ...
          {resultado.dia_ideal_esperado && (
            <div style={{ background: 'var(--green-bg)', borderRadius: 8, padding: '12px 14px', fontSize: 13 }}>
              ...
              <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                <span>Dia {resultado.dia_ideal_esperado.dia} · {fmtData(resultado.dia_ideal_esperado.data)}</span>
...
function DetalheLote({
  ...
  const handleAdicionarAnimais = async (input: { data_entrada: string; data_pesagem2?: string; linhas: LinhaAnimalInput[] }) => {
        ...
        return (
          <Modal open onClose={() => setShowProjecaoAnimal(null)} title={`Projeção — ${animal.codigo}`} size="lg">
            <PainelProjecao
              ...
              custoCompraTotal={animal.valor_compra} custoAlimentacaoAcumulado={r?.custoAcumulado ?? 0}
              ciclosConfig={ciclosFuturos} rendimentos={rendimentos} bonus={bonus}
            />
          </Modal>
        )
