...
//
// Reconstrói, para um único animal, o peso projetado e o custo de alimentação
// acumulado até uma data qualquer, considerando:
...
export function calcularAnimalNaData(
  ...
  if (diaAlvo <= diaEntrada) {
    return { peso: animal.peso_entrada, custoAcumulado: 0, diasConfinamento: 0, gmdMedio: 0 }
  }
  ...
  for (let dia = diaEntrada; dia < diaAlvo; dia++) {
    ...
    if (dietaInfo?.pct_consumo_pv_ms != null) {
      ...
      if (custoKgMs != null) {
        custoAcumulado += pesoHoje * (dietaInfo.pct_consumo_pv_ms / 100) * custoKgMs
      }
    }
  }
  ...
  return { peso: pesoFinal, custoAcumulado, diasConfinamento, gmdMedio }
}
